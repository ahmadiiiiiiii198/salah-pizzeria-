...
class BusinessHoursService {
  ...
  /**
   * Format business hours for display in footer
   */
  formatHoursForDisplay(hours: WeeklyHours): string {
    ...
    if (allSame && firstDay.isOpen) {
      return `Lun-Dom: ${firstDay.openTime} - ${firstDay.closeTime}`;
    }

    // If not all same, show individual days (simplified)
    const openDays = allDays.filter(day => day.isOpen);
    if (openDays.length === 0) {
      return 'Chiuso';
    }

    // Show most common hours
    const mostCommonHours = openDays[0];
    return `Lun-Dom: ${mostCommonHours.openTime} - ${mostCommonHours.closeTime}`;
  }
}
