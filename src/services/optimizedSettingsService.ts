...
class OptimizedSettingsService {
  ...
  /**
   * Force refresh a setting by clearing its cache and fetching fresh data
   */
  async forceRefreshSetting(key: string): Promise<any> {
    console.log(`🔄 Force refreshing setting: ${key}`);
    this.clearCache(key);
