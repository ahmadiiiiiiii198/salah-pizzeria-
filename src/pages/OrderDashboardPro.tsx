...
const OrderDashboardPro: React.FC = () => {
  ...
  const updateOrderStatus = async (orderId: string, newStatus: string) => {
    try {
      ...
      console.log('🔐 [OrderDashboardPro] Ensuring admin authentication...');
      const { ensureAdminAuth } = await import('@/utils/adminDatabaseUtils');
      const authSuccess = await ensureAdminAuth();
      if (!authSuccess) {
        console.error('❌ [OrderDashboardPro] Admin authentication failed');
        throw new Error('Admin authentication required');
      }
      console.log('✅ [OrderDashboardPro] Admin authentication verified');

      // Use the update_order_status function for consistent cross-browser admin access
      console.log('🔄 [OrderDashboardPro] Calling update_order_status function...');
