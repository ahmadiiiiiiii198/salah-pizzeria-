      ...
      try {
        ...
        if (isMock || sessionId.startsWith('cs_live_mock_') || sessionId.startsWith('cs_mock_')) {
          ...
          if (order) {
            ...
            // 🎯 AUTO-REDIRECT: Automatically redirect to order state page
            console.log('🎯 Setting up redirect to order-state for order:', order.order_number);
            setTimeout(() => {
              console.log('🎯 Executing redirect to order-state...');
              navigate(`/order-state?order=${order.order_number}&auto=true`);
            }, 2000); // Wait 2 seconds to show success message
          }

          toast({
            title: 'Payment Successful! 🌸',
            description: `Redirecting to order tracking... Amount: €${orderAmount.toFixed(2)}`,
          });
        } else {
                ...
                {isVerifying ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 pt-4">
                      ...
                      <Button
                        onClick={() => navigate(`/order-state${orderNumber ? `?order=${orderNumber}` : ''}`)}
                        variant="outline"
                        className="flex-1"
                      >
