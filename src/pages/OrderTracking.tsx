...
const OrderTracking: React.FC = () => {
  ...
  // 🎯 AUTO-LOAD: Automatically search for order when redirected from order completion
  useEffect(() => {
    const autoParam = searchParams.get('auto');
    const orderParam = searchParams.get('order');

    if (autoParam === 'true' && orderParam && !autoLoaded && isAuthenticated && user && !authLoading) {
      console.log('🎯 Auto-loading order from URL:', orderParam);
      setAutoLoaded(true);
      searchOrder();
      ...
      toast({
        ...
        description: 'Welcome to your order tracking page. Your order status will update in real-time.',
        duration: 5000,
      });
    }
  }, [searchParams, autoLoaded, isAuthenticated, user, authLoading]);
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4 bg-gradient-to-r from-pizza-orange to-red-500 bg-clip-text text-transparent">
            {searchParams.get('auto') === 'true' ? 'Order Tracking' : 'I tuoi Ordini'}
          </h1>
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            {searchParams.get('auto') === 'true'
              ? `Congratulations ${user?.user_metadata?.full_name || user?.email}! Your order has been placed successfully. Track its progress below.`
              ...
              <CardContent>
                <div className="space-y-6">
                  ...
                  <div className="relative bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 p-8 rounded-2xl border border-indigo-200 shadow-lg overflow-hidden">
                    ...
                    >
                      <div className="relative">
                        {/* Beautiful Lottie Motorcycle Animation */}
                        <div className={`transition-transform duration-300 ${
                          order.status === 'delivered' ? 'animate-bounce' : ''
                        }`}>
                          <BeautifulMotorcycleAnimation
                            isActive={order.status === 'out_for_delivery' || order.status === 'arrived' || order.status === 'preparing'}
                            size="large"
