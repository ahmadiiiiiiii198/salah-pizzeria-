...
class IOSAudioFix {
  ...
  private async createBeepBuffer(): Promise<void> {
    ...
    try {
      ...
      const duration = 1.5; // 1.5 seconds for bell sound
      const frameCount = sampleRate * duration;

      const audioBuffer = this.manager.audioContext.createBuffer(1, frameCount, sampleRate);
      const channelData = audioBuffer.getChannelData(0);

      // Generate a pleasant bell-like tone with multiple harmonics
      for (let i = 0; i < frameCount; i++) {
        ...
        channelData[i] = (fundamental + second + third) * envelope * 0.3;
      }

      this.manager.audioBuffer = audioBuffer;
      console.log('🍎 [IOSAudioFix] Pleasant bell buffer created');

    } catch (error) {
      console.error('🍎 [IOSAudioFix] ❌ Failed to create bell buffer:', error);
    }
  }
