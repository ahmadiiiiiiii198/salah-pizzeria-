 ...
 * Creates an anonymous session if none exists and validates admin localStorage auth
 */
export const ensureAdminAuth = async (): Promise<boolean> => {
  try {
    // First check if admin is authenticated via localStorage
    const adminAuth = localStorage.getItem('adminAuthenticated');
    if (adminAuth !== 'true') {
      console.warn('❌ [AdminAuth] No admin localStorage authentication found');
      return false;
    }

    console.log('✅ [AdminAuth] Admin localStorage auth verified');
    ...
    console.log('🔐 [AdminAuth] No Supabase session found, creating anonymous session for admin...');
    ...
    try {
      ...
      if (authError) {
        ...
        return true; // Return true since we have public RLS policies and admin localStorage auth
      }

      console.log('✅ [AdminAuth] Anonymous session created successfully for admin');
      console.log('🔐 [AdminAuth] Session details:', {
