...
export const useUserOrders = (): UseUserOrdersReturn => {
  ...
  const loadUserOrders = useCallback(async () => {
    ...
    try {
      ...
      console.log('📋 [USER-ORDERS] Orders data:', data?.map(o => ({
        id: o.id,
        order_number: o.order_number,
        status: o.status,
        order_status: o.order_status,
        admin_done_at: o.admin_done_at
      })));
      setOrders(data || []);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', {
        ...
        console.log('📋 [USER-ORDERS-SUB] Payload new data:', payload.new);
        console.log('📋 [USER-ORDERS-SUB] Current orders before update:', orders.map(o => ({ id: o.id, status: o.status, order_status: o.order_status })));

        // Update the specific order in the list, preserving order_items
          ...
          const updatedOrders = prevOrders.map(order =>
            order.id === payload.new.id
              ? {
                  ...order,
                  ...payload.new,
                  // Preserve order_items if not included in payload
                  order_items: payload.new.order_items || order.order_items
                }
              : order
          );
          console.log('📋 [USER-ORDERS-SUB] Updated orders:', updatedOrders.map(o => ({ id: o.id, status: o.status, order_status: o.order_status })));
          return updatedOrders;
        });

        // Force a full refresh to ensure UI updates with complete data
        setTimeout(async () => {
          console.log('📋 [USER-ORDERS-SUB] Force refreshing orders after status update...');
          try {
            await refreshOrders();
            console.log('📋 [USER-ORDERS-SUB] Refresh completed successfully');
          } catch (error) {
            console.error('📋 [USER-ORDERS-SUB] Refresh failed:', error);
          }
        }, 1000); // Increased delay to ensure database update is complete

        // Show notification
        toast({
          title: 'Ordine aggiornato!',
          description: `Lo stato del tuo ordine è cambiato a: ${payload.new.status}`,
        });
      })
      .on('postgres_changes', {
        event: 'INSERT',
