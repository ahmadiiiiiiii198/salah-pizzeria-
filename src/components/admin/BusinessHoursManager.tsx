...
const BusinessHoursManager = () => {
  ...
  const saveBusinessHours = async () => {
    try {
      ...
      // Force complete refresh to clear caches (but don't override local state)
      console.log('🔄 [BusinessHoursManager] Forcing cache refresh...');
      businessHoursService.clearCache(); // Clear cache without fetching
      console.log('✅ [BusinessHoursManager] BusinessHours cache refreshed');

      // Note: We don't refresh restaurantHoursService here because it uses opening_hours, not businessHours
      // Keep the local state as-is since we just saved it successfully
      // The next time the services are called, they'll fetch fresh data from the database
