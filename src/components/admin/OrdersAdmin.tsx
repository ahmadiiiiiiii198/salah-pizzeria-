...
const OrdersAdmin = () => {
  ...
  const markOrderAsRead = async (orderId: string) => {
    console.log('📖 [OrdersAdmin] markOrderAsRead called:', {
      orderId,
      timestamp: new Date().toISOString()
    });

    try {
      // 🎯 CRITICAL: Ensure admin authentication for cross-browser access
      console.log('🔐 [OrdersAdmin] Ensuring admin authentication...');
      const authSuccess = await ensureAdminAuth();
      if (!authSuccess) {
        console.error('❌ [OrdersAdmin] Admin authentication failed');
        throw new Error('Admin authentication required');
      }
      console.log('✅ [OrdersAdmin] Admin authentication verified');

      // First, fetch the existing order to preserve metadata
      console.log('📖 [OrdersAdmin] Fetching existing order to preserve metadata...');
      ...
      if (fetchError) {
        console.error('📖 [OrdersAdmin] Error fetching existing order:', fetchError);
        throw fetchError;
      }
      ...
      const updateData = {
        ...
        // 🎯 CRITICAL: Preserve existing metadata to ensure real-time updates work
        metadata: existingOrder?.metadata || {}
      };

      console.log('📖 [OrdersAdmin] Sending read update to database with preserved metadata:', updateData);
      ...
      console.log('📖 [OrdersAdmin] Database read update result:', { error, data });
      ...
      setOrders(prev => prev.map(order =>
        order.id === orderId
          ? { ...order, admin_read_at: new Date().toISOString() }
          : order
      ));

      toast({
        title: "✅ Letto",
        description: "Ordine contrassegnato come letto",
      });
    } catch (error) {
  ...
  const updateOrderStatus = async (orderId: string, newStatus: string) => {
    console.log('🔄 [OrdersAdmin] updateOrderStatus called:', {
      orderId,
      newStatus,
      timestamp: new Date().toISOString()
    });
    ...
    try {
      ...
      // Use the update_order_status function for consistent cross-browser admin access
      console.log('🔄 [OrdersAdmin] Calling update_order_status function...');
      const { error } = await supabase.rpc('update_order_status', {
        order_uuid: orderId,
        new_status: newStatus,
        status_notes: `Stato aggiornato a: ${newStatus}`,
        tracking_num: null,
      });

      console.log('🔄 [OrdersAdmin] Database update result:', { error });

      if (error) throw error;

      // No need to create notifications here - they should only be created for new orders
      // This prevents notification sound from playing when admin changes order status
      ...
      setOrders(prev => prev.map(order =>
        order.id === orderId
          ? {
              ...
              updated_at: new Date().toISOString(),
              ...(newStatus === 'delivered' ? { admin_done_at: new Date().toISOString() } : {})
            }
          : order
      ));
      ...
      console.log(`✅ [OrdersAdmin] Order ${orderId} status updated to ${newStatus}`);
    } catch (error) {
    ...
    const ordersSubscription = supabase
      ...
      .on('postgres_changes',
        ...
        (payload) => {
          ...
          loadOrders(false); // Reload orders when new order is added (no loading indicator)
      ...
      .on('postgres_changes',
        ...
        (payload) => {
          console.log('🔄 [OrdersAdmin] Order updated via real-time:', payload);

          // Only update if the order exists in current state to prevent unnecessary renders
          setOrders(prevOrders => {
            const orderExists = prevOrders.some(order => order.id === payload.new.id);
            if (!orderExists) {
              console.log('🔄 [OrdersAdmin] Order not in current list, skipping update');
              return prevOrders;
            }
          ...
          // CRITICAL: Also update selectedOrder if it's the same order
          if (selectedOrder && selectedOrder.id === payload.new.id) {
            console.log('🔄 [OrdersAdmin] Updating selectedOrder via real-time:', payload.new);
          ...
          {selectedOrder ? (
            <Card>
              ...
              <CardContent className="space-y-3 sm:space-y-4 p-3 sm:p-4">
                    ...
                    {orderStatuses.map((status) => {
                      ...
                      return (
                        <Button
                          ...
                          onClick={() => {
                            console.log('🔄 [OrdersAdmin] Button clicked:', {
                              status: status.value,
                          ...
                          className={`text-xs p-2 sm:p-3 h-auto transition-all duration-200 ${
                ...
                {/* Admin Actions */}
                <div>
                  <h5 className="text-sm sm:text-base font-semibold mb-2">Azioni Admin</h5>
                  <div className="flex gap-2">
                    {!selectedOrder.admin_read_at && (
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={(e) => {
                          e.preventDefault();
                          e.stopPropagation();
                          console.log('🔄 [OrdersAdmin] Mark as read clicked:', {
                            orderId: selectedOrder.id,
                            isMobile: /Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)
                          });
                          markOrderAsRead(selectedOrder.id);
                        }}
                        className="text-xs bg-blue-50 hover:bg-blue-100 border-blue-200 touch-manipulation active:bg-blue-200"
                        style={{
                    ...
                    {selectedOrder.admin_read_at && (
                      <div className="text-xs text-blue-600 bg-blue-50 px-2 py-1 rounded">
                        📖 Letto il {new Date(selectedOrder.admin_read_at).toLocaleDateString('it-IT')} alle {new Date(selectedOrder.admin_read_at).toLocaleTimeString('it-IT', { hour: '2-digit', minute: '2-digit' })}
                      </div>
                    )}
                    {selectedOrder.admin_done_at && (
                      <div className="text-xs text-green-600 bg-green-50 px-2 py-1 rounded">
                        ✅ Completato il {new Date(selectedOrder.admin_done_at).toLocaleDateString('it-IT')} alle {new Date(selectedOrder.admin_done_at).toLocaleTimeString('it-IT', { hour: '2-digit', minute: '2-digit' })}
                      </div>
                    )}
                  </div>
                </div>
