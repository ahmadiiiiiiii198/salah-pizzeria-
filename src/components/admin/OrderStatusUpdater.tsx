...
const OrderStatusUpdater: React.FC<OrderStatusUpdaterProps> = ({ 
  ...
  const updateOrderStatus = async (newStatus: string) => {
    ...
    try {
      ...
      console.log('🔐 [OrderStatusUpdater] Ensuring admin authentication...');
      const authSuccess = await ensureAdminAuth();
      if (!authSuccess) {
        console.error('❌ [OrderStatusUpdater] Admin authentication failed');
        throw new Error('Admin authentication required');
      }
      console.log('✅ [OrderStatusUpdater] Admin authentication verified');

      // Use the update_order_status function for consistent cross-browser admin access
      console.log('🔄 [OrderStatusUpdater] Calling update_order_status function...');
      const { error } = await supabase.rpc('update_order_status', {
        order_uuid: order.id,
        new_status: newStatus,
        status_notes: `Ordine #${order.order_number} aggiornato a: ${orderStatuses.find(s => s.value === newStatus)?.label}`,
        tracking_num: null,
      });
