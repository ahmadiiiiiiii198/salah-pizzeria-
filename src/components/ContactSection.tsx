      ...
      try {
        ...
        try {
          ...
          if (heroData?.value?.openingHours) {
            ...
            console.log('✅ [ContactSection] Opening hours loaded from heroContent:', heroData.value.openingHours);

            // Clear any cached versions in localStorage
            localStorage.removeItem('cachedOpeningHours');
            localStorage.removeItem('heroContent');
          } else {
            console.log('⚠️ [ContactSection] No opening hours in heroContent, using fallback');
          }
        } catch (error) {
          console.error('❌ [ContactSection] Error loading opening hours:', error);
        }
