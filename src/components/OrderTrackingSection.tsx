...
import { useHybridOrderTracking } from '@/hooks/useHybridOrderTracking';
import { getOrderItemPrice } from '@/types/order';
import { MapPin } from 'lucide-react';
import BeautifulMotorcycleAnimation from '@/components/BeautifulMotorcycleAnimation';
...
function OrderTrackingSection(): JSX.Element | null {
  ...
  // 🎯 NEW: Use hybrid tracking for better cross-browser support
  const { orders: hybridOrders, loading: hybridLoading, getActiveOrders } = useHybridOrderTracking();
  // Keep old hooks for fallback compatibility
  const { orders: userOrders, loading: userLoading } = useUserOrders();
  const { orders: clientOrders, loading: clientLoading } = useClientOrders();
  ...
  const activeOrders = useMemo(() => {
    console.log('🔍 [OrderTracking] Using NEW hybrid tracking:', {
      isAuthenticated,
      hybridOrdersCount: hybridOrders?.length || 0,
      userOrdersCount: userOrders?.length || 0,
      clientOrdersCount: clientOrders?.length || 0
    });

    // 🎯 NEW: Use hybrid tracking first, fallback to old system
    if (hybridOrders && hybridOrders.length > 0) {
      console.log('🔍 [OrderTracking] Using HYBRID orders (new system)');
      const activeHybridOrders = getActiveOrders();
      console.log('🔍 [OrderTracking] Active hybrid orders:', activeHybridOrders.length);
      return activeHybridOrders;
    }

    // 🔄 FALLBACK: Use old hybrid approach
    console.log('🔍 [OrderTracking] Falling back to old hybrid approach');
    let ordersToFilter = [];

    if (isAuthenticated && userOrders && userOrders.length > 0) {
      console.log('🔍 [OrderTracking] Using USER orders (authenticated)');
      ordersToFilter = userOrders;
    } else if (clientOrders && clientOrders.length > 0) {
      console.log('🔍 [OrderTracking] Using CLIENT orders (anonymous)');
      ordersToFilter = clientOrders;
    } else {
      console.log('🔍 [OrderTracking] No orders available from either source');
      return [];
    }

    const filtered = ordersToFilter.filter(order => {
      const currentStatus = order.status || order.order_status;
      const shouldShow = currentStatus !== 'delivered' &&
                        currentStatus !== 'cancelled' &&
                        !order.admin_done_at;

      console.log(`🔍 [OrderTracking] Order ${order.order_number}:`, {
        currentStatus,
        admin_done_at: order.admin_done_at,
        shouldShow,
        source: isAuthenticated ? 'USER' : 'CLIENT'
      });

      return shouldShow;
    });

    console.log('🔍 [OrderTracking] Active orders after filtering:', filtered.length);
    return filtered;
  }, [isAuthenticated, hybridOrders, userOrders, clientOrders, getActiveOrders]);

  const loading = hybridLoading || userLoading || clientLoading;

  console.log('🔍 [OrderTracking] Render decision:', {
    isAuthenticated,
    activeOrdersLength: activeOrders.length,
    userLoading,
    clientLoading,
    loading,
    shouldRender: activeOrders.length > 0 && !loading
  });

  // 🎯 SHOW ORDERS FOR BOTH AUTHENTICATED AND ANONYMOUS USERS
  if (!activeOrders.length || loading) {
    console.log('🔍 [OrderTracking] Not rendering - no active orders or still loading');
    return null;
  }
  ...
  return (
    <section
      className="bg-gradient-to-br from-orange-50 via-red-50 to-pink-50 relative overflow-hidden"
      style={{
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid gap-8 md:grid-cols-2 xl:grid-cols-3">
            {activeOrders.map((order) => {
              ...
              // Add debug logging
              console.log(`🔍 [OrderTracking] Rendering order ${order.order_number}:`, {
                id: order.id,
                status: order.status,
              ...
              return (
                <Card key={`${order.id}-${order.updated_at}-${currentStatus}`} className="shadow-xl hover:shadow-2xl transition-all duration-300 border-0 bg-white rounded-2xl overflow-hidden">
                  {/* Clean Card Header */}
                    ...
                    {currentStatus && (
                      <div className="bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 p-8 rounded-2xl border-2 border-blue-300 shadow-xl mb-6">
                        ...
                        <div className="flex justify-center">
                          <BeautifulMotorcycleAnimation
                            isActive={currentStatus === 'delivering' || currentStatus === 'out_for_delivery'}
                            size="large"
                            className="w-48 h-32"
                          />
                        </div>
