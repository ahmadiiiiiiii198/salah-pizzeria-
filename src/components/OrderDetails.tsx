...
const OrderDetails = ({ order, onUpdate, onDelete }: OrderDetailsProps) => {
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      ...
      console.log('🔐 [OrderDetails] Ensuring admin authentication...');
      const { ensureAdminAuth } = await import('@/utils/adminDatabaseUtils');
      const authSuccess = await ensureAdminAuth();
      if (!authSuccess) {
        console.error('❌ [OrderDetails] Admin authentication failed');
        throw new Error('Admin authentication required');
      }
      console.log('✅ [OrderDetails] Admin authentication verified');
  ...
  const handleQuickStatusUpdate = async (status: string) => {
    ...
    try {
      // 🎯 CRITICAL: Ensure admin authentication for cross-browser access
      console.log('🔐 [OrderDetails] Ensuring admin authentication for quick update...');
      const { ensureAdminAuth } = await import('@/utils/adminDatabaseUtils');
      const authSuccess = await ensureAdminAuth();
      if (!authSuccess) {
        console.error('❌ [OrderDetails] Admin authentication failed');
      ...
      console.log('✅ [OrderDetails] Admin authentication verified for quick update');
