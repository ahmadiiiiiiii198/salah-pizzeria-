-- Fix RLS policies for order deletion and admin operations
-- This migration ensures that both authenticated users and anonymous users (for admin operations) can manage orders

-- Orders table policies - Allow both authenticated and anonymous (for admin operations)
...
-- Order items table policies - Allow both authenticated and anonymous (for admin operations)
DROP POLICY IF EXISTS "Allow authenticated users to delete order items" ON order_items;
DROP POLICY IF EXISTS "Allow admin operations on order items" ON order_items;
CREATE POLICY "Allow admin operations on order items"
  ON order_items
  FOR ALL
  USING (true);

-- Order notifications table policies - Allow both authenticated and anonymous (for admin operations)
DROP POLICY IF EXISTS "Allow authenticated users to delete order notifications" ON order_notifications;
DROP POLICY IF EXISTS "Allow admin operations on order notifications" ON order_notifications;
CREATE POLICY "Allow admin operations on order notifications"
  ON order_notifications
  FOR ALL
  USING (true);

-- Order status history table policies - Allow both authenticated and anonymous (for admin operations)
DROP POLICY IF EXISTS "Allow authenticated users to delete order status history" ON order_status_history;
DROP POLICY IF EXISTS "Allow admin operations on order status history" ON order_status_history;
...
DO $$
BEGIN
    -- Orders read policy - Allow public read access
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE tablename = 'orders'
        AND policyname = 'Allow public read access to orders'
    ) THEN
    ...
    -- Order items read policy - Allow public read access
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE tablename = 'order_items'
        AND policyname = 'Allow public read access to order items'
    ) THEN
...
DO $$
BEGIN
    -- Orders update policy - Allow public update access (admin operations)
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE tablename = 'orders'
        AND policyname = 'Allow public update access to orders'
    ) THEN
